...
# Update layout
    ...
    with col2:
        create_card("Total Quantity Sold", f"${total_quantity_sold:.2f}")
    with col3:
        create_card("Average Unit Price", f"${average_unit_price:.2f}")    
    with col4: 
        create_card("Total Gross Income", f"${total_gross_income:.2f}")
    ...
    with col2:   
            ...
            gauge={'axis': {'range': [None, 6000]},
                ...
                'threshold': {'line': {'color': "red", 'width': 4}, 'thickness': 0.75, 'value': target_tax}}))
...
def get_ghana_time():
    ...
    # Convert UTC time to Ghana time
    ghana_time = utc_time.astimezone(pytz.timezone('Africa/Accra'))
    return ghana_time

# Function to create clock HTML
def create_clock_html(hour, minute, second):
    return f"""
    <div style="position: relative; width: 200px; height: 200px; border-radius: 50%; border: 2px solid black;">
        <div style="position: absolute; top: 50%; left: 50%; transform: translate(-50%, -50%); width: 6px; height: 40px; background-color: black; transform-origin: bottom;"></div>
        <div style="position: absolute; top: 50%; left: 50%; transform: translate(-50%, -50%) rotate({(hour % 12) * 30}deg); width: 4px; height: 30px; background-color: black; transform-origin: bottom;"></div>
        <div style="position: absolute; top: 50%; left: 50%; transform: translate(-50%, -50%) rotate({minute * 6}deg); width: 3px; height: 45px; background-color: black; transform-origin: bottom;"></div>
        <div style="position: absolute; top: 50%; left: 50%; transform: translate(-50%, -50%) rotate({second * 6}deg); width: 2px; height: 50px; background-color: red; transform-origin: bottom;"></div>
        <div style="position: absolute; top: 5px; left: 50%; transform: translateX(-50%); color: white; font-size: 20px;">{hour:02d}:{minute:02d}</div>
    </div>
    """

# Display the clock widget
st.write("Current time in Ghana:")
ghana_time = get_ghana_time()

# Extract hour, minute, and second
hour = ghana_time.hour
minute = ghana_time.minute
second = ghana_time.second

# Display clock watch
st.markdown(
    create_clock_html(hour, minute, second),
    unsafe_allow_html=True
)
